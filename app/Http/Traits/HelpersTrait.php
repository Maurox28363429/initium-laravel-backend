...
trait HelpersTrait
{
    ...
    protected function HelpStore($query, $data,$messages)
    {
        try {
            DB::beginTransaction();
            ...
            DB::commit();
            return [
                "message" => (isset($messages) && $messages!=null)? "Datos creados":$messages,
                "status" => 200,
                "data" => $process
            ];
        } catch (\Exception $e) {
    ...
    protected function HelpUpdate($query, $data,$messages)
    {
        try {
            DB::beginTransaction();
            $process = $query->first();
            ...
            DB::commit();
            return [
                "message" => (isset($messages) && $messages!=null)? "Datos Actualizados":$messages,
                "status" => 200,
                "data" => $process
            ];
        } catch (\Exception $e) {
